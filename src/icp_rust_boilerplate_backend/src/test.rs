...
struct Train {
    id: u64,
    departure_station: u64,
    arrival_station: u64,
    seats: std::collections::HashMap<u64, BookingStatus>,
    price: u64,
    schedule: u64,
}
...
thread_local! {
    ...
    static ADMINS_STORAGE: RefCell<StableBTreeMap<String, AdminCap, Memory>> =
        RefCell::new(StableBTreeMap::init(
...
#[ic_cdk::update]
fn init_system(admin_id: String, station_payload: StationPayload) -> Result<String, String> {
    let admin_cap = AdminCap { admin_id: admin_id.clone() };

    ADMINS_STORAGE.with(|storage| storage.borrow_mut().insert(admin_id.clone(), admin_cap));
    
    let station_id = ID_COUNTER.with(|counter| {
    ...
    Ok(format!("System initialized with admin {} and station {}", admin_id, station_id))
}

#[ic_cdk::update]
fn create_train(admin_id: String, payload: TrainPayload) -> Result<Train, Error> {
    let admin_exists = ADMINS_STORAGE.with(|storage| storage.borrow().contains_key(&admin_id));

    if !admin_exists {
        return Err(Error::UnAuthorized { msg: "Unauthorized access".to_string() });
    }

    let departure_station_exists = STATIONS_STORAGE.with(|storage| storage.borrow().contains_key(&payload.departure_station));
    let arrival_station_exists = STATIONS_STORAGE.with(|storage| storage.borrow().contains_key(&payload.arrival_station));

    if !departure_station_exists || !arrival_station_exists {
        return Err(Error::InvalidInput { msg: "Invalid station ID(s)".to_string() });
    }

    let train_id = ID_COUNTER.with(|counter| {
        let current_value = *counter.borrow().get();
        counter.borrow_mut().set(current_value + 1)
    }).expect("Cannot increment ID counter");

    let seats = (1..=payload.seat_count).map(|seat| (seat, BookingStatus::Available)).collect();
    ...
    STATIONS_STORAGE.with(|storage| {
        storage.borrow_mut().get_mut(&payload.departure_station).unwrap().train_ids.push(train_id);
    });
...
fn buy_ticket(payload: TicketPayload) -> Result<Ticket, Error> {
    let train_exists = TRAINS_STORAGE.with(|storage| storage.borrow().contains_key(&payload.train_id));

    if !train_exists {
        return Err(Error::NotFound { msg: "Train not found".to_string() });
    }

    let mut train = TRAINS_STORAGE.with(|storage| storage.borrow().get(&payload.train_id).unwrap().clone());

    if train.schedule <= time() {
        return Err(Error::TrainDeparted { msg: "Train has already departed".to_string() });
    }

    if train.seats.get(&payload.seat_number) != Some(&BookingStatus::Available) {
        return Err(Error::AlreadyBooked { msg: "Seat already booked".to_string() });
    }

    train.seats.insert(payload.seat_number, BookingStatus::Booked);
...
fn refund_ticket(ticket_id: u64) -> Result<String, Error> {
    ...
    if !ticket_exists {
        return Err(Error::NotFound { msg: "Ticket not found".to_string() });
    }
    ...
    let mut train = TRAINS_STORAGE.with(|storage| storage.borrow().get(&ticket.train_id).unwrap().clone());

    train.seats.insert(ticket.seat_number, BookingStatus::Available);
...
#[ic_cdk::update]
fn close_train(admin_id: String, payload: CloseTrainPayload) -> Result<String, Error> {
    let admin_exists = ADMINS_STORAGE.with(|storage| storage.borrow().contains_key(&admin_id));
    ...
    let train = TRAINS_STORAGE.with(|storage| storage.borrow().get(&payload.train_id).unwrap().clone());

    STATIONS_STORAGE.with(|storage| {
        let departure_station = storage.borrow_mut().get_mut(&train.departure_station).unwrap();
        departure_station.train_ids.retain(|&id| id != payload.train_id);
    });
...
fn view_station(station_id: u64) -> Result<Station, Error> {
    let station_exists = STATIONS_STORAGE.with(|storage| storage.borrow().contains_key(&station_id));

    if !station_exists {
        return Err(Error::NotFound { msg: "Station not found".to_string() });
    }

    let station = STATIONS_STORAGE.with(|storage| storage.borrow().get(&station_id).unwrap().clone());
